...
- `PLAN-G0`、`SW-G0` 与 `SW-G1` 已于 2026-08-24 接受，`SW-G3` 与 `SW-G4/SW-V0` 已于 2026-08-28 接受并完成；`mls-rs 0.56.0` 单元 D 的固定 94-package 图保留正式历史 `STOP`，A2/A3 已形成 clean revision。2026-09-02 单元 D2 对同一固定图形成 schema 2/v2 正式 Phase A `PASS`，仓库 lockfile 已提升并与 seed/evidence 一致；11 个 package 的随包许可证证据仍不足以覆盖全部声明正文及适用 copyright/NOTICE。首次 R1 与 R1b 因不同 raw 文件 `TimeoutError` 形成有效传输负向 evidence；R1c-I 随后改用同一 tree entry blob SHA 与 Git Blobs API，R1c-R 完成 6 commit/11 package mapping 并以有效 `STOP/license-evidence` 收口：9 个 `mls-rs` package 覆盖 Apache-2.0/MIT，`debug_tree` 缺 MIT 正文，`r-efi` 只覆盖 MIT、缺 Apache-2.0/LGPL-2.1-or-later 完整正文。R1d-L 独立评审随后形成 `STOP/license-disposition`：`debug_tree` 首选绑定固定对象的 R1d-U 上游澄清，`r-efi` 首选独立法律评审接受 MIT alternative 后进入 R1d-G/R1d-D；`debug_tree` 的 R1d-U-P 已形成 clean revision，R1d-U-I 也已离线实施为独立 schema 1 helper/checker，但没有联网、创建 Issue 或正式 evidence。当前公开仓库没有两包 payload，且尚未形成 mls-rs POC 产品镜像，但未来 Linux/Android/iOS/服务端/源码包仍须按实际 SBOM 与分发物关闭。该结果不重判 R1c，不是许可证不存在、法律违规或候选淘汰结论；R2 前置不满足，Phase B 禁止。OpenMLS 0.9.0 的 A6 已由 `ac01b4d` 收口；2026-09-01 单元 E 在固定 264-package 图上形成 schema 4/v4 正式 `STOP`：source/audit/feature 为零，`cargo-audit` 未发现 vulnerability 但报告一个 unmaintained 信息项，当前许可证门拒绝三个 `MPL-2.0` crate。OpenMLS 0.9.0 不进入 Phase B，`SW-G2` 未通过；`SW-V0 PASS` 只接受 harness 有效，不代表 E2EE 路线、产品软件实现、P0 或其他证据轨通过。
...
19. **`debug_tree` R1d-U-P 已冻结**：只设计向 `martypapa/debug-tree` 官方 Issues 提交的一条公开澄清，标题/正文绑定 `0.4.0`、registry checksum 与 commit，并冻结发送前重复检查、schema 1 新 evidence、单次 POST/零 retry/资源上限、歧义写入、独立只读回复 run、维护者资格、完整 MIT notice/holder、第三方例外/NOTICE、追溯适用与官方不可变 commit 验收条件；以 `docs(security): define debug tree upstream clarification` 提交为 `2879dfb32f21107914073fc820cd2450ca3951dc`。
20. **R1d-U-I 已离线实施并形成 clean revision**：只新增 `scripts/run-sw-g2-mls-rs-license-upstream-clarification.py` 与 `scripts/check-sw-g2-mls-rs-license-upstream-clarification.sh`，以 `feat(security): prepare debug tree upstream clarification` 提交为 `9c6d4dcef5d58f8708b28d6ce3ac743b895d71a8`；helper/checker SHA-256 分别为 `e1bcdfa7f2f7bff1e6d59fe771a5830815b07760d804ee373fd059572b163b27` 与 `c585f8bd60a6b5b38045f5788bb24c6f227c95898035d426831e5aa47d3dee52`。schema 1 固定 plan revision/SHA、消息摘要、账号与对象 identity、X/R 独立 final evidence、20/10 请求、最多一次 POST、5 MiB 下载、20 MiB evidence、300 秒、30 秒单请求、零 retry/redirect/proxy/background；POST 后不确定写入为 `INVALID/ambiguous-external-write`，R run 只收集活动并保持 `STOP`，接受性须另行评审。
21. **今日代码—文档复核已完成**：逐项回顾文档收口前 9 个提交及三次实现变更，R1/R1b 传输负向结论、R1c schema 2 blob identity 链与 R1d 处置边界均和代码一致；发现并修正 R1d-U-I 状态与后续事项漂移。R1d-U-P 设计文件因被 helper 按 revision `2879dfb` 与 SHA-256 `eaa3550927a44b910bc798563217d86146a77a66fd54bda00f708c0a26c4920d` 固定校验而保持逐字不变。今天没有执行 R1d-U-X/R、创建 clarification evidence、运行 Cargo/Docker 或 push。
...
1. **决定是否单独授权 R1d-U-X**：若继续 `debug_tree` 上游澄清，先确认将公开显示的 GitHub login，再逐字预审冻结标题/正文、唯一 repository、20 请求/一次 POST/5 MiB 下载/20 MiB evidence/300 秒边界与失败保留方式；这是公开外部写入，须新的 L3 明确授权，不能从 R1d-U-I 或本次 commit 授权推导。
2. **R1d-U-X 后仍停在等待点**：若 Issue 成功创建，只形成独立 `STOP/awaiting-upstream` final，不自动监控、追问、编辑或关闭；只有已观察到上游活动且另获授权时，才执行一次 R1d-U-R 只读收集。R1d-U-R 只保存活动并保持 `STOP`，回复接受性和后续 R1d-E 另案评审。
...
5. **禁止自动第四次 R1 或改写历史**：任何额外取证、依赖/分发修改或候选切换必须另行精确授权，不能扩大 selector、补写 evidence 或重判 R1c-R 的 `STOP/license-evidence`。
