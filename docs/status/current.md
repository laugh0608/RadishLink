...
6. **设计漂移风险**：探索性 T0 探针先于正式消息语义、故障模型和证据格式完成；如果直接扩展，会把测试实现误当成产品协议和生产技术栈。

## 当前优先级与暂停线

- 暂停 HaLow 硬件采购、射频发射、量产硬件和生产技术栈冻结；
- 2026-08-20 的 Docker A—B—C 结果只作为探索性证据，不是 T0/P0 阶段验收；
- 在[D0/T0/P0 软件工作计划](d0-t0-p0-plan.md)通过 G0 前，不扩展测试代码、不新增故障 profile、不安装密码依赖，也不重新运行场景；
- 下一步只评审计划、覆盖层消息语义、E2EE 候选和验证设计；获明确确认后才实施；
- 无射频软件计划不能替代首个测试地区的法规核对，二者可以并行研究但分别关门。

## 下一道决策门

1. 评审并接受软件工作计划的范围、术语、顺序和停止线（G0）；
2. 先形成消息 ID、lifetime、hop budget、custody、确认、去重、配额和崩溃恢复语义（G1）；
3. 评审 Signal/MLS 候选的许可证、平台和状态安全，形成 ADR 后才能进入 E2EE 实现（G2）；
4. 冻结确定性故障 profile、指标、证据 manifest 和判定口径（G3）；
5. 再提交精确实现/运行清单并获得授权（G4）；
6. 继续独立推进测试地区、SKU、频段、功率、带宽和天线的法规核对；在结论完成前不采购和发射。
...
第二个入口只产生 Docker/Ethernet 探索性证据，使用未加密合成载荷；脚本打印的 `T0 PASS` 只代表内部断言通过，不构成项目 T0/P0、HaLow、E2EE、距离、媒体、功耗或法规验证。
