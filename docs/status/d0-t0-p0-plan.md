...
- 状态：Draft，待评审
- 更新日期：2026-08-20
- 适用范围：D0 的无射频软件方案设计，以及进入 P0 前的 T0 准备
...
因此，脚本的 `T0 PASS` 只表示脚本内断言通过，不表示本计划、T0 或 P0 已通过。
...
评审本计划，确认 D0/P0 边界、探索性证据定位、工作顺序和暂停线。未通过 G0，不扩展代码或重新运行场景。
...
G1 只冻结语义和不变量，不冻结二进制编码、生产语言或数据库。

### G2：E2EE 与身份候选

按[端到端加密候选评审](../security/e2ee-candidate-review.md)比较 Signal 与 MLS 路线，先关闭许可证、支持平台、无中心投递映射和 crash-safe 状态问题。任何依赖安装或 spike 都需要另行授权。最终选择通过 ADR 接受；未通过 G2，只能测试合成不透明载荷，不能测试或宣称 E2EE。
...
根据 G1–G3 形成精确改动清单、命令、外部资源、副作用、预计时间和清理方式。获得授权后才实现用户态 proxy、状态机或库 spike，并运行短生命周期容器。

### G5：阶段判定

只有代码审查、单元/负向测试、三节点矩阵和证据复核都通过，才能把对应能力写为“Docker/Ethernet 软件模拟已验证”。是否进入 P0 还需要 G2 的安全路线和 D0 的其他退出条件共同满足。

## 建议工作包

| 顺序 | 工作包 | 主要产物 | 当前状态 |
| --- | --- | --- | --- |
| 1 | 证据归档与计划纠偏 | 本计划、探索性探针边界 | 文档已形成，待 G0 |
| 2 | 消息交付语义 | `docs/protocol/` 下的新草案 | 未开始 |
| 3 | E2EE/身份决策 | 候选实测计划与 ADR | 候选评审已有，未决策 |
| 4 | 故障与证据设计 | profile/manifest/判定规范 | 未开始 |
| 5 | T0 工具调整 | 经评审的最小实现 | 暂停 |
| 6 | 三节点矩阵 | 可复现结果与限制 | 暂停 |
