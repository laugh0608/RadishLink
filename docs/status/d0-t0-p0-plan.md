...
- `SW-G2` 未完成前，OpenMLS 0.9.0 当前基线不再重跑或进入 Phase B；mls-rs A0/A1 已分别形成 clean revision，下一步只限逐单元评审 C/D、`SW-V1/V2` 授权边界及只读核对。mls-rs 的下载、Docker/Cargo/网络或运行须按 C/D 另行精确授权，不得复用 OpenMLS 候选 lockfile/cache 或相邻授权，也不得由 `SW-V0 PASS` 推导重跑或授权 `SW-V3`。
