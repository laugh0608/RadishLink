...
按[端到端加密候选评审](../security/e2ee-candidate-review.md)比较 Signal 与 MLS 路线。[`SW-G2` 决策包](../security/e2ee-sw-g2-decision-package.md)已收敛候选顺序、无中心身份/投递映射、crash-safe 状态、许可证停止线、受限 spike 与接受条件。OpenMLS 0.8.1 Phase A 已生成 lockfile，但 advisory/许可证停止线触发，prepared run 的 Phase B 禁止；[`mls-rs 0.56.0` 静态门禁](../testing/sw-g2-mls-rs-spike-authorization.md)已接受但未执行；OpenMLS 0.9.0 的[Phase A 精确授权包](../testing/sw-g2-openmls-0.9-phase-a-authorization.md)在 A3 后已生成可解析 partial graph，但审计工具未在 45 分钟内准备完成，四类正式门均无结论。A4 已离线实现独立固定工具 bundle、精确只读消费与 partial evidence 收口；首次工具构建因 evidence finalizer 失败而整体无效，A5 已离线修正。新的 bundle 构建和 Phase A 仍须分别评审和授权。最终选择通过 ADR 接受；未通过 `SW-G2`，只能测试合成不透明载荷，不能测试或宣称 E2EE。
...
- `SW-G2` 未完成前，下一步仍只限在 A5 clean revision 上形成新 bundle 的精确 L3 授权、后续 Phase A 单次授权、`SW-V1/V2` 授权边界评审及只读核对；不得复用无效 run、由 `SW-V0 PASS` 推导重跑或授权 `SW-V3`。
