...
本文不修改 `tools/t0/`，不授权实现故障代理、构建代码、运行容器或安装依赖。所有数值当前是待评审的 D0 测试参数，不是产品默认值、外场指标、HaLow 性能或量产承诺。`SW-G3` 接受后，改变这些参数必须提升 profile/schema 版本，不能为让失败用例通过而原地放宽。
...
1. 评审并接受本文，完成 `SW-G3`；
2. 由 `SW-G4` 提交精确实现文件、依赖、命令、Docker 外部状态、时长和清理授权；
3. 先实现/验证 `SW-V0`，再逐项 `SW-V1`，然后 `SW-V2`；
4. 单故障 profile 全部稳定后，才能另行设计组合故障；
5. `SW-G2` 和候选运行授权通过后，才能执行 `SW-V3`；
6. 任一 FAIL 返回实现或设计根因，不增加 fallback、不延长窗口、不删除失败记录；
7. 任一 INVALID 先修 harness/环境，不能计入通过率。
...
当前只是 Draft。未经评审接受和 `SW-G4` 明确授权，不得据此扩展 `tools/t0/` 或重跑三节点场景。
