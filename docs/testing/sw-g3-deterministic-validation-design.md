...
2026-09-05 补充了文末的[实现前勘误](#2026-09-05-实现前勘误待评审)。它是待评审修订输入，未改变下列 1.0 数值、profile/schema 或既有 SW-V0 接受结果；后续受影响实现不能自行修补参数后声称符合原合同。
...
1. `SW-G3` 已接受；`SW-G4/SW-V0` 的精确实现与运行授权已于 2026-08-28 消费完毕，harness 自检通过且不构成重跑或扩展授权；
2. 后续先为 `SW-V1`、再为 `SW-V2` 分别提交精确实现文件、依赖、命令、Docker 外部状态、时长和清理授权，不继承 `SW-V0` 授权；
...
`SW-G3` 已接受，但不构成实现或运行授权；`SW-V0` 已完成也不产生持续授权。未经对应的新 `SW-G4` 明确授权，不得据此扩展 `tools/t0/` 或重跑三节点场景。

## 2026-09-05 实现前勘误（待评审）

状态：Draft；本节记录已发现的设计歧义与建议关闭方式，不追溯修改已接受的 1.0 或 SW-V0 evidence。受影响的 SW-V1/V2 尚未实施为正式场景；不得在实现中自行选择一种解释而仍声称原 profile 无变化。

| 项目 | 当前 1.0 的问题 | 建议的修订方向与接受条件 |
| --- | --- | --- |
| 断链与 retry | `250/500/1000/2000 ms` 合计 3750 ms；若它们是唯一重试机会，将早于 5 s 断链结束耗尽 | 明确初次发送是否计入 4 次、退避起算点和恢复事件是否触发发送。推荐评审“恢复事件触发一次受配额和 deadline 约束的补发”；若选择调整重试次数/退避，则重算观察窗。任何选择均须版本化，不由 runner 自动延长 |
| 正文与密文长度 | `SW-V2-LENGTH-001` 用 16385 B 密文作为超限，但产品 16 KiB 是正文上限，真实密文另有认证和封装开销 | 当前值仅保留为合成 profile；正式候选分别冻结正文、密文、信封上限，并覆盖刚好上限、上限加一、整数溢出和截断，不能直接把合成值接入 SW-V3 |
| 单变量范围 | `SW-V1-DOWN-BC-001` 的 fault 是链路断开，通过条件却同时提到 B 重启前后 | 链路断开验证与 SW-V2 重启验证分开；组合故障另设 profile，不能在一个单变量结论中暗含两种 fault |
| 恢复拓扑 | 固定 A—B—C 隔离且 B 唯一时，B 失效没有可用替代路径 | 分别定义断链排队、B 恢复后重连、存在 D/允许直连时切路；后者另设拓扑/profile，不复用 A/C 必须隔离的自检结论 |
| 确定性与覆盖 | 三次固定 seed 一致只检验所覆盖条件的可复现性 | 后续多 seed、并发、组合故障与长期运行用独立 variant/profile 和预算，不追溯覆盖 canonical 结果 |

修订包须提供具体时序图或事件表、profile/version 差异、retry 次数和 deadline 的正负边界、预期状态及受影响消费者。接受时记录新的设计修订，参数变化提升 profile version；只有结构/语义不兼容时才提升相应 schema 主版本。新的实现和正式运行分别取得明确授权，不覆盖历史输入、结果或 checksum。
