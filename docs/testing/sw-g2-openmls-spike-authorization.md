...
隔离 spike 骨架与 lockfile 已形成；Phase A 最终在许可证和 advisory 门 `STOP`，来源检查通过但安全与许可证条件未关闭。Phase B 不再是“待授权即可执行”，而是被本轮负向证据阻断；镜像与 1.5 GiB 忽略 cache 的清理也未授权。OpenMLS 0.9.0 A3 后的独立 Phase A 已生成 partial graph，但在审计工具安装期间 deadline `STOP`；A4 固定工具 bundle 尚未构建，仍不得复用任一 prepared run 进入场景。
