...
暂不二选一，也不在 T0 引入密码依赖。下一步在获得依赖安装与许可证评审授权后，分别做受限 spike：Signal 路线验证一对一异步/乱序/重启语义，MLS 路线验证两成员组与分区 epoch 处理。以同一套 A—B—C 故障矩阵比较安全、状态复杂度、平台和许可证，再由 ADR 冻结；在此之前项目继续使用“E2EE 候选/待验证”。
