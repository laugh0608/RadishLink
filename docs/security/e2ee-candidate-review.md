...
- `OpenMLS 0.9.0` 的稳定发布只解除 prerelease 停止线，不能证明旧 advisory、许可证和持久化风险已经关闭；[`SW-EXP-004` Phase A](../testing/sw-g2-openmls-0.9-phase-a-authorization.md)首轮因原固定 SQLite 依赖冲突而 `STOP`；A3 对齐后生成的 partial graph 只有 `rusqlite 0.37.0` / `libsqlite3-sys 0.35.0`，但新一轮在安装 `cargo-audit` 期间触发 45 分钟 deadline。A4 已离线实现独立固定工具 bundle 和只读消费；首次工具构建的固定二进制成功而 evidence finalizer 失败，整个 run 无效。A5 已离线修正根因但未重跑，仍无成功 bundle、新 Phase A、来源、许可证/advisory 或 feature 结论；官方安全策略只覆盖主 `openmls` crate，crypto provider 与 storage backend 必须独立审计；
...
暂不二选一，也不在 `SW-V*` 引入密码依赖。当前按[`SW-G2` 决策包](e2ee-sw-g2-decision-package.md)保留 OpenMLS 0.8.1 Phase A 负向证据；[`mls-rs 0.56.0` 静态门禁](../testing/sw-g2-mls-rs-spike-authorization.md)已接受但未执行；[OpenMLS 0.9.0 Phase A](../testing/sw-g2-openmls-0.9-phase-a-authorization.md)已确认依赖图可解析，A5 已修正固定工具 bundle 的 evidence finalizer，但必须在 A5 clean revision 上另行授权一次新构建并复核成功 bundle，才可为精确 bundle ID 申请新的 Phase A。`libsignal v0.101.0` 在许可证和 Linux ARM64 集成面关闭前仍只做静态核对。只有候选通过精确依赖、advisory、许可证、命令、副作用和运行授权，才以同一套已接受 `SW-G3` A—B—C 故障矩阵比较安全、状态复杂度、平台和许可证，再由 ADR 冻结；在此之前项目继续使用“E2EE 候选/待验证”。
