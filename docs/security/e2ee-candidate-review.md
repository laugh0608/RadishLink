...
本评审属于[D0/T0/P0 软件工作计划](../status/d0-t0-p0-plan.md)的 G2 输入。G0/G1 未完成前不启动库 spike；G2 未形成 ADR 前不把任何候选接入 T0/P0。

本评审不自行拼装密码原语，不用 TLS/WPA3 代替应用层 E2EE，也不因 T0 合成明文通过而宣称 B 无法读取内容。
...
暂不二选一，也不在 T0 引入密码依赖。先完成工作计划 G0 和消息语义 G1，再为 G2 提交受限 spike 设计、许可证评审范围、精确依赖和运行授权：Signal 路线验证一对一异步/乱序/重启语义，MLS 路线验证两成员组与分区 epoch 处理。以同一套经 G3 评审的 A—B—C 故障矩阵比较安全、状态复杂度、平台和许可证，再由 ADR 冻结；在此之前项目继续使用“E2EE 候选/待验证”。
