...
- 状态：Draft（OpenMLS 0.8.1 与 0.9.0 固定图均为 Phase A 负向 `STOP`；mls-rs D 保留历史 `STOP`，A2/A3 已形成 clean revision；D2 与 Phase B 未授权）
- 资料核对日期：2026-09-02
- 适用 gate：`SW-G2`
...
| `mls-rs` | `0.56.0` | Apache-2.0 OR MIT；Rust；提供 storage traits、SQLite provider、互操作与 FFI/UniFFI 路径 | D 为历史 feature-gate `STOP`；A2 只纠正 core alias 的过度约束，完整第三方审计、X.509 支持面、资源边界和 Linux ARM64 实证仍未关闭 | 对照候选，不是后备默认值；A3 已形成 clean revision，D2 未授权 |
...
单元 D 的正式历史 `STOP` 与 A2/A3 的离线修订都不授权执行以下场景。A3 已形成 clean revision，只读消费 D final lock，不重新解析传递版本或复用旧 mutable cache；仍须由独立 D2 授权对固定依赖图形成有效 Phase A `PASS`，并完成人工许可证与非实现者 evidence 复核。此前 Phase B 继续禁止。
