...
- [`SW-EXP-004` OpenMLS 0.9.0 静态门禁与执行授权包](testing/sw-g2-openmls-0.9-spike-authorization.md)：A3 已关闭原 SQLite 解析冲突；A5 后固定审计工具 bundle 已 `PASS`，精确 Phase A 尚未执行。
- [`SW-EXP-004` OpenMLS 0.9.0 实施骨架与 Phase A 精确授权包](testing/sw-g2-openmls-0.9-phase-a-authorization.md)：成功 bundle `20260830-112214-39636.GpERrj` 已复核；L3 单元 D 授权包已形成但未获执行授权，Phase B 禁止。
