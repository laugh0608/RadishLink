...
- [`SW-EXP-003` mls-rs 0.56.0 许可证/NOTICE 补证与非实现者复核包](testing/sw-g2-mls-rs-license-notice-review.md)：R0/helper 已固定 11 个随包证据不足 package、3 个上游仓库、6 个不可变 commit、受限联网来源与证据合同；首次 R1 与同合同 R1b 均因不同 raw 文件的 `TimeoutError` 有效 `STOP/license-evidence`，不是许可证缺失结论。当前只形成改用同一 commit tree blob SHA 与 Git Blobs API 的 R1c 设计，实施、运行、R2 与 Phase B 均未授权。
