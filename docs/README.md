...
- [`SW-EXP-004` OpenMLS 0.9.0 静态门禁与执行授权包](testing/sw-g2-openmls-0.9-spike-authorization.md)：A3 已关闭原 SQLite 解析冲突；新一轮 Phase A 生成 partial graph 后在审计工具安装期间触发 45 分钟 deadline。
- [`SW-EXP-004` OpenMLS 0.9.0 实施骨架与 Phase A 精确授权包](testing/sw-g2-openmls-0.9-phase-a-authorization.md)：实施单元 A/A2/A3 已完成；当前只有 evidence-only lockfile，无正式审计结论，Phase B 禁止。
