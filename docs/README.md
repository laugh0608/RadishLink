...
- [`SW-EXP-003` mls-rs 0.56.0 许可证/NOTICE 补证与非实现者复核包](testing/sw-g2-mls-rs-license-notice-review.md)：R0/helper 已固定 11 个随包证据不足 package、3 个上游仓库、6 个不可变 commit、受限联网来源与证据合同；首次 R1 因 `TimeoutError` 有效 `STOP/license-evidence`，不是许可证缺失结论，R1b/R2 与 Phase B 未授权。
