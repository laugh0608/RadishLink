...
- [`SW-EXP-003` mls-rs 0.56.0 静态门禁与执行授权包](testing/sw-g2-mls-rs-spike-authorization.md)：静态候选方向已接受；单元 D 在固定 94-package 图的 transitive feature gate 正式 `STOP`，Phase B 禁止。
- [`SW-EXP-003` mls-rs 0.56.0 实施与 Phase A 精确授权包](testing/sw-g2-mls-rs-phase-a-authorization.md)：A0/A1 已分别形成 clean revision，单元 C 固定工具 bundle 已 `PASS`；单元 D 因 `mls-rs-core` 默认启用 `fast_serialize` / `rfc_compliant` 正式 `STOP`，未提升仓库 lockfile。
