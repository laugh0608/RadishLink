...
offline_checker_path="${repo_root}/scripts/check-sw-g2-mls-rs-phase-a.sh"
spike_relative="tools/spikes/sw-g2-mls-rs"
...
for command_name in awk basename chmod cp date df docker find git id jq ln mkdir mktemp mv pwd python3 rg rm shasum sleep sort tee uname; do
...
if [ -n "${preflight_git_status}" ]; then
  echo "STOP: mls-rs Phase A requires a clean worktree" >&2
  exit 10
fi
...
if [ ! -x "${audit_tool_bundle_bin}/cargo-audit" ] || [ ! -x "${audit_tool_bundle_bin}/cargo-deny" ] ||
  [ -w "${audit_tool_bundle_bin}/cargo-audit" ] || [ -w "${audit_tool_bundle_bin}/cargo-deny" ]; then
  echo "audit tool bundle binaries must be executable and host read-only" >&2
  exit 2
fi
...
if [ "$(find "${audit_tool_bundle_bin}" -mindepth 1 -maxdepth 1 -type f | awk 'END { print NR + 0 }')" -ne 2 ]; then
  echo "audit tool bundle must contain exactly two regular binaries" >&2
  exit 2
fi
...
if ! jq -e \
  ...
  --arg deny_version "${cargo_deny_version}" '
    ...
    and (.input_sha256.runtime_control_helper | test("^[0-9a-f]{64}$"))
    and (.input_sha256.builder | test("^[0-9a-f]{64}$"))
    and (.input_sha256.manifest_filter | test("^[0-9a-f]{64}$"))
    and (.input_sha256.offline_checker | test("^[0-9a-f]{64}$"))
  ...
  echo "audit tool bundle manifest does not satisfy the fixed generic contract" >&2
  exit 2
fi

bundle_checksum_prefix="artifacts/sw-g2-rust-audit-tools/${audit_tool_bundle_id}"
...
container_name="radishlink-sw-g2-mls-rs-${run_id}"
...
mkdir -p "${work_dir}/cargo-home" "${work_dir}/cargo-target" "${prepared_spike}/src"
: > "${run_log}"
...
write_checksums() {
  ...
  append_checksum "${runtime_control_helper}" "scripts/monitor-sw-g2-dependency-audit-run.py" "${checksums_tmp}"
  ...
  for evidence_name in \
    ...
    append_checksum "${run_dir}/${evidence_name}" "artifacts/sw-g2-mls-rs/${run_id}/${evidence_name}" "${checksums_tmp}"
  done
...
expected_git_status_after() {
  if [ "${lockfile_written}" = true ]; then
    printf '?? %s' "${lock_relative}"
  else
    printf '%s' ""
  fi
}
...
load_partial_dependency_state() {
  ...
  if [ -f "${run_dir}/advisory-db-revision.txt" ] && [ ! -L "${run_dir}/advisory-db-revision.txt" ]; then
...
verify_pass_phase_a_contract() {
  ...
  expected_status="$(expected_git_status_after)"
  ...
  if [ "$(shasum -a 256 "${repo_lock}" | awk '{print $1}')" != "${lock_sha}" ] ||
    ...
    echo "STOP: PASS contract requires identical repository and evidence lockfiles" >&2
    return 1
  fi
  if ! jq -e \
    --arg contract "${manifest_contract}" \
    ...
    --arg expected_status "${expected_status}" '
      ...
      and .input_sha256.phase_a_manifest_filter == $filter_sha
      and .repository.git_status_before == ""
      and .repository.git_status_after == $expected_status
      and .runtime_controls.termination_reason == "completed"
...
cleanup() {
  ...
  if [ "${workflow_exit_code}" -eq 0 ] && [ "${final_stage}" = "phase-a-prepared" ] &&
    [ "${source_status}" = "0" ] && [ "${audit_status}" = "0" ] &&
    [ "${deny_status}" = "0" ] && [ "${feature_status}" = "0" ] &&
    [ "${container_residual_count}" = "0" ] && [ "${git_status_before}" = "" ] &&
    [ "${git_status_after}" = "${expected_status}" ] && inputs_unchanged; then
    runtime_control_status="stopped"
    runtime_termination_reason="completed"
    manifest_outcome="PASS"
    manifest_stage="phase-a-prepared"
  else
  ...
  if ! write_manifest "${workflow_exit_code}" "${manifest_outcome}" "${manifest_stage}"; then
    echo "STOP: Phase A manifest finalization failed" >&2
    workflow_exit_code=1
    manifest_outcome="INVALID"
  elif ! write_checksums; then
    echo "STOP: Phase A checksum finalization failed" >&2
    workflow_exit_code=1
    manifest_outcome="INVALID"
    preserve_invalid_finalization
  elif [ "${manifest_outcome}" = "PASS" ]; then
    if verify_pass_phase_a_contract; then
      echo "SW-EXP-003 PHASE A PASS: ${run_id}"
    else
      workflow_exit_code=1
      manifest_outcome="INVALID"
      preserve_invalid_finalization
  ...
  if [ "${manifest_outcome}" != "PASS" ]; then
    echo "SW-EXP-003 PHASE A ${manifest_outcome}: ${run_id}" >&2
  fi
  echo "Evidence retained at artifacts/sw-g2-mls-rs/${run_id}"
...
echo "[1/8] record clean revision, fixed inputs, host, daemon, and disk"
...
if ! [[ "${disk_available_kib}" =~ ^[0-9]+$ ]] || [ "${disk_available_kib}" -lt "${minimum_disk_kib}" ]; then
  echo "STOP: less than 5 GiB is available for mls-rs Phase A" >&2
  exit 10
fi
...
if ! run_controlled docker ps -a --filter "label=${label_key}=${run_id}" --format '{{.ID}}' > "${work_dir}/preexisting-containers.txt"; then
  echo "STOP: could not inspect exact mls-rs run residuals" >&2
  exit 10
fi
if [ -s "${work_dir}/preexisting-containers.txt" ]; then
  echo "STOP: exact mls-rs run label already has residual containers" >&2
  exit 10
fi

echo "[2/8] verify immutable Linux ARM64 image"
...
host_uid="$(id -u)"
if [ "${host_uid}" = "0" ]; then
  echo "STOP: mls-rs Phase A requires a non-root host user" >&2
  exit 12
fi
host_user="${host_uid}:$(id -g)"

echo "[3/8] verify Linux ARM64 Rust toolchain without network"
...
echo "[4/8] create isolated candidate input copy"
current_stage="isolate-inputs"
...
run_controlled docker run --rm \
  ...
  sh -euc '
    ...
    printf "{\"source\":%s,\"audit\":%s,\"deny\":%s}\n" \
      "$source_status" "$audit_status" "$deny_status" > /evidence/audit-exit-codes.json
...
if [[ "${feature_status}" =~ ^[0-9]+$ ]] && [ "${feature_status}" -ne 0 ]; then
  ...
  echo "STOP: mls-rs feature/provider/source resolution did not match the accepted gate" >&2
  exit 24
fi

echo "[7/8] record advisory revision and apply source/feature lockfile gate"
current_stage="lockfile-promotion"
load_partial_dependency_state
